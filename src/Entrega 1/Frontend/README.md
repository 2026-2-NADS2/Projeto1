...
- O menu lateral tem uma cor para cada perfil, seguindo as asas da borboleta do símbolo.
...
Os dados usados são fictícios e ficam em `src/data/mock` (dentro da pasta `Frontend`), organizados conforme as tabelas do banco do projeto. As telas buscam esses dados pelo arquivo `src/services/api.js`, que depois será ligado ao backend.

## Estrutura

Dentro de `src/Entrega 1/Frontend`:
...
| Cor | Código | Uso |
|---|---|---|
| Azul marinho | `#1B2A4A` | Cor da marca: logo, títulos e menu lateral do Administrador |
| Azul petróleo | `#0E6E85` | Botões e elementos interativos |
| Azul petróleo (hover) | `#0A5A6D` | Botões ao passar o mouse |
| Dourado | `#E8B33D` | Alertas e menu lateral do Professor |
| Magenta | `#A82486` | Menu lateral do Responsável |
...
As cores do menu lateral seguem as asas da borboleta do símbolo da 4U:

| Perfil | Cor do menu |
|---|---|
| Administrador | Azul marinho `#1B2A4A` |
| Professor | Dourado `#E8B33D` (texto em azul marinho) |
| Responsável | Magenta `#A82486` |
